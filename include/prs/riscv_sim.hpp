...
#define PRS_RISCV_MAP_OP_INFO_TO_OP(op_info, op, matcher)                      \
  template <> struct op_traits<op_info> {                                      \
    using op_type = op;                                                        \
...
using riscv_operand_description_t =
    operand_description_t<reg_operand_info_t, imm_operand_info_t,
                          csr_operand_info_t>;
#endif

class riscv_operand_description_t;
template <> struct op_desc_traits<riscv_operand_description_t> {
  using op_info_t = riscv_operand_info_t;
  using op_matcher_t = riscv_operand_matcher_t;
};

class riscv_operand_description_t
    : private operand_description_t<riscv_operand_description_t> {
public:
  using op_info_t = riscv_operand_info_t;
  using op_matcher_t = riscv_operand_matcher_t;

  using operand_description_t::operand_description_t;
  using operand_description_t::visit;

  using per_enc_t =
      std::unordered_map<encoding_id_t,
                         std::vector<riscv_operand_description_t>>;
...
template <>
inline constexpr std::string_view opcode_wrapper_t<opcode_t>::to_str() const {
  return opc2str(opcode);
}

using riscv_decoded_inst_t = decoded_inst_t<opcode_t, riscv_operand_t>;

template <> inline void riscv_decoded_inst_t::print(logger_t l) const {
  auto opc_str = opcode.to_str();
  for (auto c : opc_str)
    l << static_cast<char>(std::tolower(c));

  for (auto &op : operands)
    op.visit([&](reg_operand_t reg) { l << "\tx" << reg.get_reg(); },
...
class inst_description_t {
  ...
  std::vector<riscv_operand_description_t> op_descs;
...
public:
  ...
  std::vector<riscv_operand_t> operands(inst_t inst) const {
    std::vector<riscv_operand_t> operands;
    for (const auto &op_desc : op_descs) {
      op_desc.visit(
          [inst, &operands](reg_operand_info_t reg_info,
                            reg_operand_matcher_t reg_matcher) {
            operands.push_back(
                riscv_operand_t(reg_info, reg_matcher.extract(inst)));
          },
          [inst, &operands](imm_operand_info_t imm_info,
                            const imm_operand_matcher_t &imm_matcher) {
            operands.push_back(
                riscv_operand_t(imm_info, imm_matcher.extract(inst)));
          },
          [inst, &operands](csr_operand_info_t csr_info,
                            csr_operand_matcher_t csr_matcher) {
            operands.push_back(
                riscv_operand_t(csr_info, csr_matcher.extract(inst)));
          },
...
class simulator_t {
  ...
  const static std::unordered_map<
      opcode_t, void (*)(simulator_t &, std::span<const prs::riscv_operand_t>)>
      i_opc_actions;
