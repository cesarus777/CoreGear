...
public:
  ...
  auto visit(this self_t &&self, fs_t &&...funcs) {
    ...
    return std::visit(overloaded(std::forward<fs_t>(funcs)...), op_info_v,
                      op_matcher_v);
  }

  using per_enc_t =
      std::unordered_map<encoding_id_t,
                         std::vector<operand_description_t<traits>>>;
